...
class MLService {
  constructor() {
    ...
    this.faceApiUrl = process.env.FACE_API_URL || 'http://localhost:8004';  // Face recognition API URL
  }
  ...
  async enrollFace(userId, imageFilePaths) {
    try {
      // Face recognition enrollment with multiple images
      const formData = new FormData();
      
      // Append all image files
      imageFilePaths.forEach((imagePath, index) => {
  ...
  async verifyFace(userId, imageFilePath) {
    try {
      const formData = new FormData();
  ...
  async checkHealth() {
    const results = {
      ...
      keystroke: false,
      mouse: false,
      face: false  // Face recognition health status
    };
