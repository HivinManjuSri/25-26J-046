...
Write-Host "This script will start all 4 ML API services in separate terminals." -ForegroundColor Yellow
...
Write-Host "  4. Face Recognition API (Port 8004)" -ForegroundColor White
Write-Host ""
...
Write-Host "Starting Face Recognition API (Port 8004)..." -ForegroundColor Green
$facePath = Join-Path $rootPath "face recognition"
Start-Process powershell -ArgumentList "-NoExit", "-Command", "cd '$facePath'; if (Test-Path 'venv\Scripts\Activate.ps1') { .\venv\Scripts\Activate.ps1 }; python main.py api"
Start-Sleep -Seconds 3
...
Write-Host "  - Face API: http://localhost:8004/health" -ForegroundColor White
Write-Host ""
